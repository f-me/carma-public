...
module Carma.EraGlonass.Test.Types.EGVinOperationAcceptCode
     ( spec
     ) where
  ...
  describe "Correctness of JSON parser" $ do
    it "All correct cases" $
      ...
      forM_ [minBound..maxBound] $ \x -> case x of
        ...
        VinNotFound ->
          parseEither jsonParser (String "VIN_NOT_FOUND") `shouldBe` Right x

    it "Fails on empty value" $ do
      let substr = findSubstring "expected EGVinOperationAcceptCode"
    ...
    it "Incorrect JSON type" $ do
      let substr = findSubstring
            "expected EGVinOperationAcceptCode, encountered Number"
  ...
  describe "Correctness of JSON encoder" $
    it "All correct cases" $
      ...
      forM_ [minBound..maxBound] $ \x -> case x of
        ...
        VinNotFound     -> toJSON x `shouldBe` String "VIN_NOT_FOUND"


jsonParser :: Value -> Parser EGVinOperationAcceptCode
