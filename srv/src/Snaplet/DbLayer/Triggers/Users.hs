...
-- | Special handling for new user meta: read login&password fields
-- from commit, create new user, then strip these fields from commit,
-- store @uid@ with newly created user id in commit and proceed. Must
-- be used with @usermeta@ commits only.
...
createUsermetaTrigger obj = do
  ...
  case uRes of
    ...
    Right newUser -> do
        ...
        return $ M.delete "login"
               $ M.delete "password"
               $ M.insert "uid" (encodeUtf8 uid) obj


-- | If @password@ field is present in a @usermeta@ commit, then
-- change password of the corresponding user and strip @password@ from
-- commit. Must be used with @usermeta@ commits only.
updateUsermetaTrigger :: ObjectId -> Object -> DbHandler b Object
updateUsermetaTrigger objId obj = do
  case M.lookup "password" obj of
    Nothing -> return obj
    Just password -> do
      ...
      case uRes of
        ...
        Just user -> do
             uRes' <- with auth $ withBackend $
                      \bk -> liftIO $ setPassword user password >>= save bk
             case uRes' of
               Left e -> error $
