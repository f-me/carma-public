...
pg_dump -h ${FROMHOST} -U carma_db_sync ${FROMDB} -t '"Contract_tmp"' -t '"VinFormat_tmp"' -t '"CarMake_syn"' -t '"CarModel_syn"' -t '"Engine_syn"' -t '"Transmission_syn"' -t '"SubProgram_syn"' > /tmp/Transfer.sql
psql -h ${FROMHOST} -U carma_db_sync ${FROMDB} -c 'DROP TABLE "Contract_tmp","VinFormat_tmp","CarMake_syn","CarModel_syn","Engine_syn","Transmission_syn","SubProgram_syn";'
...
DROP TABLE "Contract_tmp","VinFormat_tmp","CarMake_syn","CarModel_syn","Engine_syn","Transmission_syn","SubProgram_syn";
EOF
)
