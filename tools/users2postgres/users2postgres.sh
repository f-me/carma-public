...
# Create users & metas
cat ${users} | jq '.uidCache | .[][1] | {login: .login, password: (null | tostring), isActive: "1", realName: (.meta.realName // ("")), roles: (.roles | map (.+",") | add )} | tostring' | sed -e 's/,\\"/\\"/' | sed -e "s/\(.*\)/curl -X POST localhost:${CARMA_PORT}\/_\/usermeta\/ --data \1/" > ${http_commands}
