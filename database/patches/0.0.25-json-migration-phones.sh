...
curl -s "http://localhost:${CARMA_PORT}/all/partner" | \
    jq -c -r '.[] | .id + "|\(if .phone1 and ((.phone1) == "" | not) then [{key:("disp"), value:.phone1, note: (if .workingTime and ((.workingTime) == "" | not) then .workingTime else "" end)}] else [] end + if .salesPhone and ((.salesPhone) == "" | not) then [{key:("sales"), value:.salesPhone, note: (if .salesWorking and ((.salesWorking) == "" | not) then .salesWorking else "" end)}] else [] end + if .servicePhone and ((.servicePhone) == "" | not) then [{key:("serv"), value:.servicePhone, note: (if .serviceWorking and ((.serviceWorking) == "" | not) then .serviceWorking else "" end)}] else [] end + if .closeTicketPhone and ((.closeTicketPhone) == "" | not) then [{key:("close"), value:.closeTicketPhone}] else [] end + if .fax and ((.fax) == "" | not) then [{key:("fax"), value:.fax}] else [] end)"' > ${OUTPUT}
