...
UPDATE "SubProgram" s
SET smsContact = t.value
FROM "SmsTokenValue_tmp" t
WHERE t.pValue = s.value AND t.tok = 1;

UPDATE "SubProgram" s
SET smsSender = t.value
FROM "SmsTokenValue_tmp" t
WHERE t.pValue = s.value AND t.tok = 2;

UPDATE "SubProgram" s
SET smsProgram = t.value
FROM "SmsTokenValue_tmp" t
WHERE t.pValue = s.value AND t.tok = 3;

DROP TABLE "SmsTokenValue";
DROP TABLE "SmsTokenName";
