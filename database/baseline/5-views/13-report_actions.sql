...
create view report_actions as
 SELECT c.id::text ||
        CASE
            WHEN first_value(COALESCE(serv.id, (-1))) OVER (PARTITION BY c.id ORDER BY COALESCE(serv.id, (-1))) <> first_value(serv.id) OVER (PARTITION BY c.id ORDER BY COALESCE(serv.id, (-1)) DESC) THEN '/'::text || dense_rank() OVER (PARTITION BY c.id ORDER BY COALESCE(serv.id, (-1)))::character varying(50)::text
            ELSE ''::text
